...
Set_config_port(){
	...
	if [ $auto != 1 ]; then
        ...
        read -e -p "(默认: 48888):" ssr_port
    fi

	[[ -z "$ssr_port" ]] && ssr_port="48888"
...
Set_config_password(){
    if [ $auto != 1 ]; then
        ...
        read -e -p "(默认: song123456):" ssr_password
    fi

	[[ -z "${ssr_password}" ]] && ssr_password="song123456"
...
read -e -p "是否自动安装(y/s): " auto
# 自动安装 auto=1
[[ "$auto" == "y" ]] || [[ "$auto" == "yes" ]] && auto=1
if [ $auto == 1 ]; then
    Install_SSR
    exit 1
fi
...
————————————
 ...
 ${Green_font_prefix}16.${Font_color_suffix} 烤鸭的自动安装
 "
menu_status
